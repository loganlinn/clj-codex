...
(defn read! "Read a thread's current goal." [c t]
  (:goal (api/invoke! c {:op :thread.goal/get :args {:thread-id (::thread/id (thread/ref t))}})))
(defn set! "Set or update a goal. A new objective resets usage accounting." [c t fields]
  (:goal (api/invoke! c {:op :thread.goal/set :args (assoc fields :thread-id (::thread/id (thread/ref t)))})))
(defn clear! "Clear a thread's goal." [c t]
  (api/invoke! c {:op :thread.goal/clear :args {:thread-id (::thread/id (thread/ref t))}}))
(defn remaining-tokens "Remaining token budget, or nil for an unbudgeted goal." [goal]
