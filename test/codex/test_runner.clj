(ns codex.test-runner
  (:require [clojure.test :as t]
            [codex.impl.websocket.loading-test]))
...
(defn -main [& args]
  ...
  (let [loading (t/run-tests 'codex.impl.websocket.loading-test)
        ...
        babashka? (some? (System/getProperty "babashka.version"))
        ...
        result (when-not loading-only?
                 ...
                 (when babashka?
                   (require 'codex.review-json-example-test))
                 (when supported?
                 ...
                 (apply t/run-tests
                        (cond-> ['codex.core-test 'codex.transport-test 'codex.review-test]
                          babashka? (conj 'codex.review-json-example-test)
