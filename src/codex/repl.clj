   ...
   (let [events (atom []) out *out*
         handler (fn [e]
                   (swap! events #(let [v (conj % e)]
                                    (if (> (count v) history-limit)
                                      (into [] (subvec v 1))
                                      v)))
                   (when print? (binding [*out* out]
                                  (pprint/pprint e)
                                  (flush))))]
     {:subscription (event/listen! c opts handler)
      :events events})))

(defn unwatch!
  "Stop a REPL watcher."
  [watch]
  (some-> watch :subscription event/unlisten!))

(defn history
  "Return the bounded event history of a watcher."
  [watch]
  (some-> watch :events deref))
...
(defn inspect
  ...
  [value]
  (let [v (if (instance? codex.app_server.Connection value)
            (server/info value)
            value)]
    (pprint/pprint v)
    v))
